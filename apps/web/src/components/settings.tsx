...
import { clearByokCredentials } from "../lib/provider";
...
export function SettingsScreen({ t, locale, connection, cache, onLocale, onRefresh, onManageProvider }: { t: T; locale: Locale; connection: ConnectionState; cache: UseYoutubeCache; onLocale: () => void; onRefresh: () => void; onManageProvider: () => void }) {
  ...
  const rows: Array<[string, string]> = [[t("settings.worker"), connection.status], ["FFmpeg", cap?.ffmpeg ? t("project.ready") : t("settings.unavailable")], ["FFprobe", cap?.ffprobe ? t("project.ready") : t("settings.unavailable")], ["yt-dlp", cap?.ytDlp ? t("project.ready") : t("settings.unavailable")], [t("settings.gateway"), cap?.gatewayConfigured ? t("project.ready") : t("settings.unavailable")], [t("settings.vision"), visionLabel(t, cap?.vision)], [t("settings.encoder"), cap?.encoders.join(", ") || "—"], [t("settings.formats"), cap?.supportedFormats?.join(", ") || "—"], [t("settings.dataRoot"), cap?.dataRoot ?? "—"], [t("settings.outputRoot"), cap?.outputRoot ?? "—"]];
  return <section className="settings-screen content-width"><header><span className="eyebrow"><span />{t("sidebar.settings")}</span><h1>{t("settings.title")}</h1><p>{t("settings.copy")}</p></header>
    {cap?.provider && <ProviderPanel t={t} provider={cap.provider} configured={Boolean(cap.providerConfigured)} onManageProvider={onManageProvider} onRefresh={onRefresh} />}
    <div className="settings-layout"><section className="panel"><div className="settings-row"><span>{t("settings.language")}</span><button className="secondary-button" onClick={onLocale}><Globe2 size={15} />{locale.toUpperCase()}</button></div>{rows.map(([label, value]) => <div key={label} className="settings-row"><span>{label}</span><b title={value}>{value}</b></div>)}</section><aside className="panel settings-note"><Gauge size={22} /><b>{t("settings.renderInfo")}</b><p>{t("settings.renderInfoCopy")}</p>{isDemoMode && <div className="demo-storage-note"><HardDrive size={15} /><span>{t("settings.cacheDemo")}</span></div>}<button className="secondary-button" onClick={onRefresh}><RefreshCw size={15} />{t("settings.recheck")}</button></aside></div>
    {!isDemoMode && <section className="panel cache-manager">
